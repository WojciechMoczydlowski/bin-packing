...
class Unpacker(object):
    ...
    :param file_like:
        ...
        If specified, unpacker reads serialized dataController from it and :meth:`feed()` is not usable.
    ...
    :param str unicode_errors:
        ...
        This option should be used only when you have msgpack dataController which
        contains invalid UTF-8 string.

    :param int max_buffer_size:
        Limits size of dataController waiting unpacked.  0 means 2**32-1.
        The default value is 100*1024*1024 (100MiB).
        Raises `BufferFull` exception when it is insufficient.
        You should set this parameter when unpacking dataController from untrusted source.
...
class Packer(object):
    ...
    def pack_ext_type(self, typecode, data):
        ...
        if not isinstance(data, bytes):
            raise TypeError("dataController must have bytes type")
        L = len(data)
        if L > 0xFFFFFFFF:
            raise ValueError("Too large dataController")
        if L == 1:
            self._buffer.write(b"\xd4")
        elif L == 2:
            self._buffer.write(b"\xd5")
